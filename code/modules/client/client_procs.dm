	...
	//SECURITY//
	////////////
#define UPLOAD_LIMIT 8*1024*1024	//Restricts client uploads to the server to 8MB //Could probably do with being lower.
...
/client/New(TopicData)
	...
	if (isnum(cached_player_age) && cached_player_age == -1) //first connection
		if (nnpa >= 0)
			...
			if (CONFIG_GET(flag/irc_first_connection_alert))
				send2tgs("New-user", "[key_name(src)] is connecting for the first time!")
	...
	if(account_age >= 0 && account_age < nnpa)
		...
		if (CONFIG_GET(flag/irc_first_connection_alert))
			send2tgs("new_byond_user", "[key_name(src)] (IP: [address], ID: [computer_id]) is a new BYOND account [account_age] day[(account_age==1?"":"s")] old, created on [account_join_date].")
